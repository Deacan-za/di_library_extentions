        ...
        private static void AddServicesFromAssemblies(this IUnityContainer container, params Assembly[] assemblies)
        {
            foreach (var assembly in assemblies)
            {
                var installerType = assembly.DefinedTypes.Where(x =>
                  typeof(IServiceInjector<IUnityContainer>).IsAssignableFrom(x)
                  && !x.IsInterface
                  && !x.IsAbstract);

                var installers = installerType.Select(Activator.CreateInstance).Cast<IServiceInjector<IUnityContainer>>();
